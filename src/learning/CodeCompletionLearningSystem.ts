...
export class CodeCompletionLearningSystem {
  ...
  getSuggestions(
    ...
    const afterCursor = currentCode.substring(
      cursorPosition,
      Math.min(currentCode.length, cursorPosition + 50)
    );
  ...
  private matchesContext(currentContext: string, pattern: CodePattern): boolean {
    ...
    if (
      normalizedContext.includes(
        normalizedPattern.substring(0, Math.min(normalizedPattern.length, 20))
      )
    ) {
      return true;
    }
  ...
  private extractKeywords(text: string): string[] {
    ...
    const keywords =
      text.match(
        /\b(function|const|let|var|if|for|while|class|import|export|return|async|await)\b/g
      ) || [];
  ...
  private extractFunctionPatterns(
    content: string,
    language: string
  ): Array<{ pattern: string; completion: string }> {
