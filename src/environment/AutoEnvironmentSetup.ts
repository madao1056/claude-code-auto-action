...
export class AutoEnvironmentSetup {
  ...
  async setupEnvironment(): Promise<void> {
    ...
    if (this.config.autoDetect) {
      ...
      console.log(`✅ プロジェクトタイプを検出: ${this.projectInfo.type} (${this.projectInfo.framework || 'vanilla'})`);
    }
  ...
  private async detectProjectType(): Promise<ProjectInfo> {
    ...
    // composer.jsonを確認（PHP）
      ...
      const composer = JSON.parse(fs.readFileSync(path.join(this.projectRoot, 'composer.json'), 'utf-8'));
...
.mypy_cache`;
  ...
  private async setupDatabase(): Promise<void> {
    ...
    if (fs.existsSync(path.join(this.projectRoot, 'docker-compose.yml'))) {
      try {
        await execAsync(`docker-compose up -d ${this.projectInfo.database}`, { cwd: this.projectRoot });
...
jobs:
  test:
    ...
    services:
      ${this.projectInfo.database ? `${this.projectInfo.database}:
