...
export class TaskDistributor extends EventEmitter {
  ...
  constructor(strategy: DistributionStrategy = {
    name: 'intelligent',
  ...
  private initializeTaskTemplates(): void {
    ...
    this.taskTemplates.set('analyze_codebase', {
      ...
      description_pattern: 'Perform comprehensive analysis of {component} including architecture, patterns, dependencies, and potential issues',
    ...
    this.taskTemplates.set('identify_dependencies', {
      ...
      description_pattern: 'Map out all dependencies, version compatibility, security vulnerabilities, and upgrade paths',
    ...
    this.taskTemplates.set('implement_feature', {
      ...
      description_pattern: 'Develop {feature_name} according to specifications, including tests, documentation, and error handling',
    ...
    this.taskTemplates.set('fix_bug', {
      ...
      description_pattern: 'Investigate, reproduce, and fix the bug: {bug_description}. Include regression tests.',
    ...
    this.taskTemplates.set('create_tests', {
      ...
      description_pattern: 'Develop unit, integration, and end-to-end tests for {component} with >90% coverage',
    ...
    this.taskTemplates.set('create_documentation', {
      ...
      description_pattern: 'Write comprehensive documentation including API docs, usage examples, and troubleshooting guides',
  ...
  private generateAnalysisTasks(analysis: ProjectAnalysis, counter: { value: number }): Task[] {
    ...
    tasks.push(this.createTaskFromTemplate('analyze_codebase', {
    ...
    // Dependency analysis
    tasks.push(this.createTaskFromTemplate('identify_dependencies', {
      id: `task_${++counter.value}`,
  ...
  private generateImplementationTasks(analysis: ProjectAnalysis, counter: { value: number }): Task[] {
    const tasks: Task[] = [];

    // Feature implementation tasks
    analysis.modifications_needed.forEach(modification => {
      tasks.push(this.createTaskFromTemplate('implement_feature', {
        id: `task_${++counter.value}`,
  ...
  private generateTestingTasks(analysis: ProjectAnalysis, counter: { value: number }): Task[] {
    ...
    analysis.technologies.forEach(tech => {
      tasks.push(this.createTaskFromTemplate('create_tests', {
        id: `task_${++counter.value}`,
        variables: { component: `${tech} components` },
        priority: 'medium'
      }));
  ...
  private generateDocumentationTasks(analysis: ProjectAnalysis, counter: { value: number }): Task[] {
    const tasks: Task[] = [];

    // Documentation for each requirement
    analysis.documentation_needs.forEach(docNeed => {
      tasks.push(this.createTaskFromTemplate('create_documentation', {
        id: `task_${++counter.value}`,
  ...
  private createTaskFromTemplate(templateKey: string, options: {
    id: string;
  ...
  private distributeByCapability(task: Task, agents: AgentConfig[]): AgentConfig | null {
    const template = Array.from(this.taskTemplates.values())
      .find(t => t.type === task.type);
    ...
    const suitableAgents = agents.filter(agent => 
      template.required_capabilities.every(cap => 
  ...
  private getPerformanceScore(agentId: string, taskType: string): number {
    const relevantHistory = this.taskHistory.filter(h => 
      h.agentId === agentId && h.success
    );

    if (relevantHistory.length === 0) return 0.5;

    const successRate = relevantHistory.length / 
      this.taskHistory.filter(h => h.agentId === agentId).length;
    
    const avgDuration = relevantHistory.reduce((sum, h) => sum + h.duration, 0) / 
  ...
  getDistributionStats(): any {
    ...
    for (const [agentId, stats] of agentStats) {
      ...
      stats.avgDuration = agentRecords.reduce((sum, r) => sum + r.duration, 0) / agentRecords.length;
    }
