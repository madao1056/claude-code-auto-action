...
export class AutoVersionManager {
  ...
  async bumpVersion(
    explicitBump?: 'major' | 'minor' | 'patch' | 'prerelease'
  ): Promise<VersionBump> {
  ...
  private async getCommitsSinceLastTag(): Promise<CommitInfo[]> {
    try {
      ...
      const { stdout } = await execAsync(`git log ${range} --pretty=format:"%H|%s|%b|%D"`, {
        cwd: this.projectRoot,
      });
  ...
  private generateChangelogEntry(version: string, commits: CommitInfo[]): ChangelogEntry {
    ...
    for (const commit of commits) {
      const message = commit.scope ? `**${commit.scope}:** ${commit.subject}` : commit.subject;
  ...
  private async createGitHubRelease(version: string, notes: string): Promise<void> {
    ...
    try {
      await execAsync(`gh release create ${tag} --title "Release ${version}" --notes "${notes}"`, {
        cwd: this.projectRoot,
      });
  ...
  private async generateReleaseArtifacts(version: string): Promise<void> {
    ...
    try {
      ...
      // tarballを作成
      await execAsync(`tar -czf release-${version}.tar.gz -C ${artifactsDir} .`, {
        cwd: this.projectRoot,
      });
  ...
  async validateVersion(): Promise<boolean> {
    ...
    // タグとの整合性をチェック
    try {
      const { stdout } = await execAsync(`git tag -l "${this.config.tagPrefix}${currentVersion}"`, {
        cwd: this.projectRoot,
      });
